...
/// Driver for the Ism330is sensor.
///
...
#[bisync]
impl<P, T> Ism330is<i2c::I2cBus<P>, T, MainBank>
where
    P: I2c,
    T: DelayNs,
{
...
#[bisync]
impl<P, T> Ism330is<spi::SpiBus<P>, T, MainBank>
where
    P: SpiDevice,
    T: DelayNs,
{
...
#[bisync]
impl<B, T, S> MemBankFunctions<MemBank> for Ism330is<B, T, S>
where
    B: BusOperation,
    T: DelayNs,
    S: BankState,
...
#[bisync]
impl<B: BusOperation, T: DelayNs, S: BankState> SensorOperation for Ism330is<B, T, S> {
    type Error = Error<B::Error>;
...
#[bisync]
impl<B: BusOperation, T: DelayNs> Ism330is<B, T, MainBank> {
...
#[cfg(feature = "passthrough")]
#[only_sync]
pub struct Ism330isMaster<B, T>
where
    B: BusOperation,
    T: DelayNs,
{
    pub sensor: RefCell<Ism330is<B, T, MainBank>>,
}

#[cfg(feature = "passthrough")]
#[only_sync]
impl<B: BusOperation, T: DelayNs> Ism330isMaster<B, T> {
    pub fn from_bus(bus: B, tim: T) -> Self {
        Self {
            sensor: RefCell::new(Ism330is::from_bus(bus, tim)),
        }
    }
    ...
    pub fn as_passthrough<'a>(&'a self, address: SevenBitAddress) -> Ism330isPassthrough<'a, B, T> {
        Ism330isPassthrough {
            sensor: &self.sensor,
...
#[cfg(feature = "passthrough")]
#[only_sync]
pub struct Ism330isPassthrough<'a, B, T>
where
    B: BusOperation,
    T: DelayNs,
{
    sensor: &'a RefCell<Ism330is<B, T, MainBank>>,
    slave_address: SevenBitAddress,
}

#[cfg(feature = "passthrough")]
#[only_sync]
// Ism330is acts like a bus when become the master of the sensor hub.
impl<B, T> BusOperation for Ism330isPassthrough<'_, B, T>
where
    B: BusOperation,
    T: DelayNs,
{
...
where
    ...
    T: DelayNs,
{
    sensor: &'a mut Ism330is<B, T, MainBank>,
    slave_address: SevenBitAddress,
}

#[cfg(feature = "passthrough")]
#[only_async]
impl<'a, B, T> Ism330isPassthrough<'a, B, T>
where
    B: BusOperation,
    T: DelayNs,
{
